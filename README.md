...
| Task | Command |
| --- | --- |
| Optimize images | `wp optimize-images ./images` |
| Audit only | `wp optimize-images audit ./images` |
| Sync output with source | `wp optimize-images sync ./images` |
| Configure TinyPNG | `wp optimize-images configure` |
| Show environment/status | `wp optimize-images status` |
| Show package version | `wp optimize-images version` |
...
By default, output is written to a sibling `optimized-images` directory. Unchanged files are skipped on future runs.

## Commands

### Optimize

Optimize all supported files in a source directory:
...
Use custom processing settings:

```bash
wp optimize-images ./images --quality=82 --max-width=2200 --format=webp
```
...
Configure the global TinyPNG API key:
...
Show the current environment and package configuration:
...
- PNG alpha/transparency preservation when converting to WebP
...
| Preset | Max dimensions | Local quality |
| --- | ---: | ---: |
| `web` | 1920 × 1920 px | 75 |
| `default` | 2880 × 2880 px | 80 |
| `retina` | 3840 × 3840 px | 85 |
...
SVG files remain SVG and continue to use SVGO. PNG transparency is preserved during WebP conversion.

Temporarily disable a project-level WebP conversion setting:

```bash
wp optimize-images ./images --format=original
```

Combine conversion with a preset:
...
If two source files would produce the same output filename, processing stops instead of overwriting one of them. For example, `logo.jpg` and `logo.png` cannot both be converted to `logo.webp` in the same directory.
...
Resize values must be positive integers and cannot exceed `20,000 px`. SVG files are never resized.
...
Absolute paths are supported as well. The output directory cannot be inside the source directory.
...
Preview processing without changing files:
